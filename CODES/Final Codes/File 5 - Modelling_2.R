...
early_legal <- early_legal %>% 
  ...
  # mutate(FICO_poor = if_else(case_cbr_score <= 579, case_cbr_score, 0),
  #        FICO_fair = if_else(case_cbr_score > 579 & case_cbr_score <= 669, case_cbr_score, 0),
  #        FICO_good = if_else(case_cbr_score > 669 & case_cbr_score <= 739, case_cbr_score, 0),
  #        FICO_very_good = if_else(case_cbr_score > 739 & case_cbr_score <= 799, case_cbr_score, 0),
  #        FICO_exceptional = if_else(case_cbr_score > 799, case_cbr_score, 0))
  #   
# Detecting the quantile values (cut-off values) 
quantile_values <- quantile(early_legal$total_case_exposure, probs = c(0, 0.25, 0.5, 0.75, 1))
quantile_values

# Create quantile
quantiles_cat <- cut(early_legal$total_case_exposure, breaks = quantile_values, labels = FALSE, include.lowest = TRUE)

# Joining it back to the data-set
early_legal <- cbind(early_legal, quantiles_cat)

# Dummies for each categories 
early_legal <- early_legal %>% 
  mutate(dum_exposure_Q1 = case_when(quantiles_cat == 1 ~ 1,
                                    T ~ 0),
         dum_exposure_Q2 = case_when(quantiles_cat == 2 ~ 1,
                                    T ~ 0),
         dum_exposure_Q3 = case_when(quantiles_cat == 3 ~ 1,
                                    T ~ 0),
         dum_exposure_Q4 = case_when(quantiles_cat == 4 ~ 1,
                                    T ~ 0))
#### Check #####

# Assuming 'sca_exposure' is the column in your dataset
sca_exposure <- early_legal$sca_exposure

# Calculate quartiles
quartile_values <- quantile(sca_exposure, probs = c(0, 0.25, 0.5, 0.75, 1), include.lowest = TRUE)

# Create quartile dummies
el_exposure_Q1 <- ifelse(sca_exposure <= quartile_values[2], sca_exposure, 0)
el_exposure_Q2 <- ifelse(sca_exposure > quartile_values[2] & sca_exposure <= quartile_values[3], sca_exposure, 0)
el_exposure_Q3 <- ifelse(sca_exposure > quartile_values[3] & sca_exposure <= quartile_values[4], sca_exposure, 0)
el_exposure_Q4 <- ifelse(sca_exposure > quartile_values[4], sca_exposure, 0)

# Combine the new variables with the original dataset
early_legal <- cbind(early_legal, el_exposure_Q1, el_exposure_Q2, el_exposure_Q3, el_exposure_Q4)

# Looking the summary
table(early_legal$quantiles_cat)
table(early_legal$small_business)
...
# Data-set for Consumer and Small Business
...
model_gen_eng_consumer_co <- glm(pvcDUM ~ sca_co_tca_due + sca_co_exposure + sca_co_tp_due + 
                                co_setup_cdss + co_tsr_max_prob + 
                                phone_rpc + phone_inbound +  phone_outbound + chat + sms + 
                                email_sent + email_letter_sent + mobile + web + both, data = data_cons, family = binomial(link = "logit"))

summary(model_gen_eng_consumer_co)
...
model_gen_eng_business_co <- glm(pvcDUM ~ sca_co_tca_due + sca_co_exposure + sca_co_tp_due + 
                                co_setup_cdss + co_tsr_max_prob + 
                                phone_rpc + phone_inbound +  phone_outbound + chat + sms + 
                                email_sent + email_letter_sent + mobile + web + both, data = data_sm, family = binomial(link = "logit"))

summary(model_gen_eng_business_co)
...
stargazer(model_gen_eng_all_co, model_gen_eng_consumer_co, model_gen_eng_business_co, 
          type="text",
          dep.var.labels = "Not Placed")

stargazer(model_gen_eng_all, model_gen_eng_consumer, model_gen_eng_business, 
          type="text",
          dep.var.labels = "Not Placed")


############# Early Legal: General + Engagements (Before and After EL)  ######################
early_legal$cons_FICO <- early_legal$case_cbr_score * early_legal$cons * early_legal$fico_categories
early_legal$sm_bus_FICO <- early_legal$case_cbr_score * early_legal$small_business
...
for (i in 1:nrow(hyper_grid)) {
  ...
  rf_tune <- 
    ranger(
      formula         = pvcDUM ~ sca_co_tca_due + sca_co_exposure + sca_co_tp_due + 
                         co_setup_cdss + co_tsr_max_prob + small_business +
                         cons_co_FICO + sm_bus_co_FICO, 
...
ggplot(early_legal, 
  ...
  labs(title = "CO_OVERALL: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "CO_OVERALL: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# plot all ROC curves
plot(roc_rf, main="CO_OVERALL: ROC Curves Comparison", col="#377eb8", lwd=2)
...
#1.2. CASE OPEN -- GENERAL: SMALL BUSINESS 
...
# predict into sample
data_sm$notplaced_pred <- predict(rf_final, data_sm)$predictions[,1]
data_sm$placed_pred <- predict(rf_final, data_sm)$predictions[,2]
...
ggplot(data_sm, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "CO_SMALL BUSINESS: Actual Not Placed Vs Predicted Not Placed Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "CO_SMALL BUSINESS: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compute the ROC curve
roc_rf <- roc(data_sm$pvcDUM, data_sm$notplaced_pred)
...
# Compare RF to Logit
# output predictions
data_sm$logit_pred <- predict(model4, data_sm, "response")

# calculate ROC curves
roc_logit <- roc(data_sm$pvcDUM, data_sm$logit_pred)

# plot all ROC curves
plot(roc_rf, main="CO_SMALL BUSINESS: ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
...
#1.3. CASE OPEN -- GENERAL: CONSUMER
...
# predict into sample
data_cons$notplaced_pred <- predict(rf_final, data_cons)$predictions[,1]
data_cons$placed_pred <- predict(rf_final, data_cons)$predictions[,2]
...
ggplot(data_cons, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "CO_CONSUMER: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "CO_CONSUMER: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compute the ROC curve
roc_rf <- roc(data_cons$pvcDUM, data_cons$notplaced_pred)
...
# Compare RF to Logit
# output predictions
data_cons$logit_pred <- predict(model3, data_cons, "response")

# calculate ROC curves
roc_logit <- roc(data_cons$pvcDUM, data_cons$logit_pred)

# plot all ROC curves
plot(roc_rf, main="CO_CONSUMER: ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
...
#################### 2.EARLY LEGAL -- GENERAL ###################
# 2.1. EARLY LEGAL -- GENERAL: OVERALL
...
ggplot(early_legal, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "EL_GENERAL: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "EL_GENERAL: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compare RF to Logit
# output predictions
early_legal$logit_pred <- predict(model11, early_legal, "response")

# calculate ROC curves
roc_logit <- roc(early_legal$pvcDUM, early_legal$logit_pred)

# plot all ROC curves
plot(roc_rf, main="EL_GENERAL: ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
...
#2.2 EARLY LEGAL -- GENERAL: SMALL BUSINESS 
...
ggplot(data_sm, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "EL_SMALL BUSINESS: Actual Not Placed Vs Predicted Not Placed Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "Variable Importance Measures: SMALL BUSINESS",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compare RF to Logit
# output predictions
data_sm$logit_pred <- predict(model33, data_sm, "response")

# calculate ROC curves
roc_logit <- roc(data_sm$pvcDUM, data_sm$logit_pred)

# plot all ROC curves
plot(roc_rf, main="Small Business: ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
...
# 2.3. EARLY LEGAL -- GENERAL: CONSUMER 
...
ggplot(data_cons, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "EL_CONSUMER: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "EL_CONSUMER: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Plot the ROC curve
# plot(roc_rf, main="EL-CONSUMER: ROC Curve for Not Placed Prediction",
#      col="#377eb8", lwd=2)
# abline(a=0, b=1, lty=2, col="gray")
# 
# auc(roc_rf)
...
# Compare RF to Logit
# output predictions
data_cons$logit_pred <- predict(model22, data_cons, "response")

# calculate ROC curves
roc_logit <- roc(data_cons$pvcDUM, data_cons$logit_pred)

# plot all ROC curves
plot(roc_rf, main="EL_CONSUMER: ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
...
# 3.1. GENERAL + ENGAGEMENT : OVERALL (Before)
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "BEFORE EL: OVERALL(GEN+ENG): Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# 3.1.1. GENERAL + ENGAGEMENT : OVERALL (AFTER)
...
ggplot(early_legal, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "AFTER EL: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "AFTER EL:OVERALL(GEN+ENG): Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compare RF to Logit
# output predictions
early_legal$logit_pred <- predict(model_after, early_legal, "response")

# calculate ROC curves
roc_logit <- roc(early_legal$pvcDUM, early_legal$logit_pred)

# plot all ROC curves
plot(roc_rf, main="AFTER EL:OVERALL(GEN+ENG):ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
...
#3.2. GENERAL + ENGAGEMENT: SMALL BUSINESS 
...
ggplot(data_sm, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "Small Business: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "SMALL BUSINESS: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compare RF to Logit
# output predictions
data_sm$logit_pred <- predict(model_gen_eng_business, data_sm, "response")
...
# 3.3. GENERAL + ENGAGEMENT: CONSUMER 
...
for (i in 1:nrow(hyper_grid)) {
  ...
  rf_tune <- 
    ranger(
      formula         =  vcDUM ~ sca_co_tca_due + sca_co_exposure + sca_co_tp_due + 
        co_setup_cdss + co_tsr_max_prob + 
        phone_rpc + phone_inbound +  phone_outbound + chat + sms + 
        email_sent + email_letter_sent + mobile + web + both, 
...
ggplot(data_cons, 
  ...
  geom_jitter(alpha = 0.5) +
  geom_hline(yintercept = 0.5, color = "red", linetype = "dashed") +
  labs(title = "CONSUMER: Not Placed (Actual Vs Predicted) Probabilities",
       x = "Actual Not Placed", 
       y = "Predicted Not Placed Probability") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
ggplot(var_importance, aes(x = reorder(Variable, Importance), y = Importance)) +
  ...
  geom_point(size = 3) + 
  coord_flip() +
  labs(title = "CONSUMER: Variable Importance Measures",
       y = "Importance Metric",
       x = "Variable") +
  theme_bw() +
  theme(panel.border = element_blank(),
...
# Compare RF to Logit
# output predictions
data_cons$logit_pred <- predict(model_gen_eng_consumer, data_cons, "response")

# calculate ROC curves
roc_logit <- roc(data_cons$pvcDUM, data_cons$logit_pred)

# plot all ROC curves
plot(roc_rf, main="CONSUMER: ROC Curves Comparison", col="#377eb8", lwd=2)
lines(roc_logit, col="red", lwd=2)
# lines(roc_probit, col="green", lwd=2)
legend("bottomright", legend=c("RF", "Logit"),
       col=c("#377eb8", "red"), lty=1, lwd=2) 
