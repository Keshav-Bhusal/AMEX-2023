...
# Creating the bins for EXPOSURE and FICO at Early Legal
early_legal <- early_legal %>%   
...
export(early_legal, "working_data/updated_version.RData")
...
# cv.tree() determine the optimal level of tree complexity; cost complexity 
...
# the classification error rate to guide the cross-validation and pruning process, 
# rather than the default for the cv.tree() function, which is deviance.
# prune the tree to obtain the nine-node tree (Lowest CV Error: 
# The tree with 9 terminal nodes results in only 74 cross-validation errors )

# Creating a feasible data-set
early_legal1 <- early_legal %>% 
  select(-pseudo_key, -case_grp_cd, -case_seq_nbr, - act_dt, -act_type_cd, - act_let_cd, 
         -case_open_dt, -case_close_dt, -age_num_days,- message_date, -message_DPB, -portfo_w_lvl_cd,
         -co_exposure_to_tp_due, -outbound_success_rate) %>% 
  filter(!is.na(case_cbr_score)) %>% 
  filter(!is.na(co_case_cbr_score)) %>% 
  filter(age_el_to_caseclose != 0) %>% 
  na.omit()
...
bag.el <- randomForest(pvcDUM ~ pvcDUM ~ sca_tca_due + sca_exposure + sca_tp_due + 
